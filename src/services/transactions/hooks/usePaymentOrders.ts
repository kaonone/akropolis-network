...
import { c2fcContractName } from 'shared/constants';
...
export default function usePaymentOrders(tokenId: string): IChildrenProps {
  ...
  const contract = drizzle.contracts[c2fcContractName];
  const contractState = drizzleState.contracts[c2fcContractName];
