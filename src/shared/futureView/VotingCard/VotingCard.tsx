    ...
    withdraw: () => (
      <>
        ...
        <Grid item xs container direction="column">
          ...
          <Typography variant="h6" className={classes.address}>{shortenString((votingParams as IWithdrawVoting).addressTo, 10)}</Typography>
        </Grid>
      </>
    ),
    join: () => (
      <Grid item xs container direction="column">
        ...
        <Typography variant="h6" className={classes.address}>{shortenString((votingParams as IJoinVoting).address, 10)}</Typography>
      </Grid>
    ),
  };
  ...
  return (
    <Grid className={classes.root} container wrap="nowrap">
      ...
      <Grid item xs={3} className={classes.voting}>
        <Grid container spacing={16}>
          <Grid item xs={12}><VotingProgress title={t(tKeysShared.yes.getKey())} value={voteForCount} type="for" /></Grid>
          <Grid item xs={12}><VotingProgress title={t(tKeysShared.no.getKey())} value={voteAgainstCount} type="against" /></Grid>
          {!votingDecision && (
            <>
              <Grid item xs={6}>
